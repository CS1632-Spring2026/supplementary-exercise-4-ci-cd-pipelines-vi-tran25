...
public class RentACatImpl implements RentACat {
		...
		for (Cat c : cats) {
			if (c.getId() == id) {
				if (c.getRented()) {
					c.returnCat();
					System.out.println(
						String.format("Welcome back, %s!", c.getName()));
					return true;
				} else {
					System.out.println(
						String.format("%s is already here!", c.getName()));
					return false;
				}
			}
		}
	...
	public boolean rentCat(int id) {
		for (Cat c : cats) {
			if (c.getId() == id) {
				if (c.getRented()) {
					System.out.println(
						String.format("Sorry, %s is not here!", c.getName()));
					return false;
				} else {
					c.rentCat();
					System.out.println(
						String.format("%s has been rented.", c.getName()));
					return true;
				}
			}
		}
	...
	public String listCats() {
		ArrayList<String> availableCat = new ArrayList<String>();
		for (Cat c : cats) {
			if (!c.getRented()) {
				availableCat.add(c.toString());
			}
		}

		if (availableCat.isEmpty()) return "";
